...
def save_models_enhanced(self, filepath='continuous_game_models.pkl'):
        ...
        with self.get_conn() as conn:
            ...
            for _, change in pending_changes.iterrows():
                try:
                    ...
                    # Calculate impact
                    players_before_7d = before_7d.iloc[0]['avg_playing'] if len(before_7d) > 0 and not pd.isna(before_7d.iloc[0]['avg_playing']) else 0
                    players_after_7d = after_7d.iloc[0]['avg_playing'] if len(after_7d) > 0 and not pd.isna(after_7d.iloc[0]['avg_playing']) else 0
                    players_before_30d = before_30d.iloc[0]['avg_playing'] if len(before_30d) > 0 and not pd.isna(before_30d.iloc[0]['avg_playing']) else 0
                    players_after_30d = after_30d.iloc[0]['avg_playing'] if len(after_30d) > 0 and not pd.isna(after_30d.iloc[0]['avg_playing']) else 0
                    ...
                    if abs(performance_impact) > 0.01:  # Only log significant changes
    ...
    def update_dynamic_keywords(self):
        ...
        with self.get_conn() as conn:
            ...
            if len(popular_games) == 0:
                print("[keywords] No games found for keyword analysis")
                return
            ...
            gaming_patterns = [
                r'\b(\w*simulator\w*)\b', r'\b(\w*tycoon\w*)\b', r'\b(\w*obby\w*)\b',
                r'\b(\w*roleplay\w*)\b', r'\b(\w*rp\w*)\b', r'\b(\w*adventure\w*)\b',
                r'\b(\w*survival\w*)\b', r'\b(\w*racing\w*)\b', r'\b(\w*fighting\w*)\b',
                r'\b(\w*horror\w*)\b', r'\b(\w*anime\w*)\b', r'\b(\w*adopt\w*)\b',
                r'\b(\w*pet\w*)\b', r'\b(\w*story\w*)\b', r'\b(\w*world\w*)\b',
                r'\b(\w*life\w*)\b', r'\b(\w*battle\w*)\b', r'\b(\w*war\w*)\b',
                r'\b(\w*school\w*)\b', r'\b(\w*hospital\w*)\b', r'\b(\w*restaurant\w*)\b',
                r'\b(\w*hotel\w*)\b', r'\b(\w*city\w*)\b', r'\b(\w*island\w*)\b',
                r'\b(\w*prison\w*)\b', r'\b(\w*zombie\w*)\b', r'\b(\w*magic\w*)\b',
                r'\b(\w*fantasy\w*)\b', r'\b(\w*space\w*)\b', r'\b(\w*ninja\w*)\b',
                r'\b(\w*pirate\w*)\b', r'\b(\w*kingdom\w*)\b', r'\b(\w*empire\w*)\b'
            ]
            ...
            if successful_text:
                ...
                # Method 3: Bigrams from successful games
    ...
    def extract_text_features_enhanced(self, text):
        ...
        high_value_keywords = self.get_high_value_keywords()
        features['high_value_keyword_count'] = sum(1 for keyword in high_value_keywords if keyword in text_lower)
    ...
    def get_high_value_keywords(self):
        """Get keywords with high success correlation"""
        try:
            with self.get_conn() as conn:
                query = """
                SELECT keyword FROM dynamic_keywords
                WHERE success_correlation > 1.3
    ...
    def create_enhanced_training_dataset(self, min_snapshots=5):
        ...
        for idx, row in df.iterrows():
            ...
            try:
                ...
                game_features['avg_favorites'] = row['avg_favorites'] or 0
                game_features['avg_likes'] = row['avg_likes'] or 0
                game_features['avg_dislikes'] = row['avg_dislikes'] or 0
                ...
                # Derived metrics
                total_votes = game_features['avg_likes'] + game_features['avg_dislikes']
                game_features['like_ratio'] = game_features['avg_likes'] / total_votes if total_votes > 0 else 0.5
                game_features['engagement_rate'] = total_votes / game_features['avg_visits'] if game_features['avg_visits'] > 0 else 0
    ...
    def calculate_adaptive_thresholds(self, df):
        ...
        return {
            ...
            'engagement': max(
                df['avg_likes'].sum() / max(df['avg_likes'].sum() + df['avg_dislikes'].sum(), 1), 
                0.7
            )
        }
...
def save_training_metadata(self, results, df):
    ...
    def predict_game_success(self, game_id):
        ...
        try:
            ...
            image_change_features = self.get_image_change_features(game_id)
            game_features.update(image_change_features)
            ...
            feature_vector_imputed = self.scalers['imputer'].transform(feature_vector)
            feature_vector_scaled = self.scalers['main'].transform(feature_vector_imputed)
            ...
            dt_pred = self.models['decision_tree'].predict(feature_vector_imputed)[0]
            dt_prob = self.models['decision_tree'].predict_proba(feature_vector_imputed)[0]
            ...
            prediction_result = {
                ...
                'key_features': {
                    ...
                    'icon_art_style': icon_features.get('art_style', 'unknown'),
                    'thumb_art_style': thumb_features.get('art_style', 'unknown'),
