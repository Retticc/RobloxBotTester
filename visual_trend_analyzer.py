#!/usr/bin/env python3
"""
visual_trend_analyzer.py - Visual Trend Analysis for Icons & Thumbnails
"""
...
class VisualTrendAnalyzer:
    ...
    def create_style_summary_image(self, style_games, style_name, style_data, image_type):
        ...
        try:
            ...
            # Plot example images (placeholders)
...
• Games Analyzed: {style_data['performance_score_count']}
            ...
            with conn.cursor() as cur:
                for asset in top_assets:
                    try:
                        ...
                        """, (
                            asset['game_id'], asset['game_name'], asset['asset_type'],
                            asset['performance_rank'], asset['performance_score'],
                            asset['visual_style'], asset['dominant_colors'],
                            asset['visual_features'], asset['success_metrics'],
    ...
    def save_trend_analysis(self, trend_report, icon_trends, thumbnail_trends):
        """Save comprehensive trend analysis to files"""
        print("[save] Saving trend analysis reports...")
        
        # Save main report as JSON
        with open(f"{self.output_dir}/reports/trend_report_{datetime.now().strftime('%Y%m%d')}.json", 'w') as f:
            json.dump(trend_report, f, indent=2, default=str)
        
        # Save detailed trend data
        if icon_trends:
            pd.DataFrame.from_dict(icon_trends, orient='index').to_csv(
                f"{self.output_dir}/reports/icon_trends_{datetime.now().strftime('%Y%m%d')}.csv"
            )
        
        if thumbnail_trends:
            pd.DataFrame.from_dict(thumbnail_trends, orient='index').to_csv(
                f"{self.output_dir}/reports/thumbnail_trends_{datetime.now().strftime('%Y%m%d')}.csv"
            )
        
        # Create summary visualization
        self.create_trend_visualization(trend_report, icon_trends, thumbnail_trends)
        
        print("[save] Trend analysis reports saved successfully")
    
    def create_trend_visualization(self, trend_report, icon_trends, thumbnail_trends):
        """Create visual trend summary charts"""
        try:
            fig, axes = plt.subplots(2, 2, figsize=(16, 12))
            fig.suptitle('Visual Trend Analysis Summary', fontsize=16)
            
            # Icon performance by style
            if icon_trends:
                icon_df = pd.DataFrame.from_dict(icon_trends, orient='index')
                icon_df = icon_df.head(8)  # Top 8 styles
                
                axes[0, 0].barh(range(len(icon_df)), icon_df['performance_score_mean'])
                axes[0, 0].set_yticks(range(len(icon_df)))
                axes[0, 0].set_yticklabels([style.replace('_', ' ').title() for style in icon_df.index])
                axes[0, 0].set_xlabel('Performance Score')
                axes[0, 0].set_title('Top Icon Styles by Performance')
            
            # Thumbnail performance by style
            if thumbnail_trends:
                thumb_df = pd.DataFrame.from_dict(thumbnail_trends, orient='index')
                thumb_df = thumb_df.head(8)  # Top 8 styles
                
                axes[0, 1].barh(range(len(thumb_df)), thumb_df['performance_score_mean'])
                axes[0, 1].set_yticks(range(len(thumb_df)))
                axes[0, 1].set_yticklabels([style.replace('_', ' ').title() for style in thumb_df.index])
                axes[0, 1].set_xlabel('Performance Score')
                axes[0, 1].set_title('Top Thumbnail Styles by Performance')
            
            # Color trends
            if 'trending_characteristics' in trend_report and trend_report['trending_characteristics'].get('color_trends'):
                color_data = trend_report['trending_characteristics']['color_trends']
                
                categories = ['Brightness', 'Saturation']
                values = [color_data.get('avg_brightness_value', 0), color_data.get('avg_saturation_value', 0)]
                
                axes[1, 0].bar(categories, values, color=['orange', 'blue'])
                axes[1, 0].set_title('Trending Color Characteristics')
                axes[1, 0].set_ylabel('Average Value')
            
            # Style characteristics
            if icon_trends and thumbnail_trends:
                all_trends = {**icon_trends, **thumbnail_trends}
                style_chars = {
                    'Has Faces': np.mean([data['has_faces'] for data in all_trends.values()]) * 100,
                    'Minimalist': np.mean([data.get('minimalist_pct', 0) for data in all_trends.values()]),
                    'High Brightness': np.mean([data['brightness_mean'] > 150 for data in all_trends.values()]) * 100,
                    'High Saturation': np.mean([data['saturation_mean'] > 60 for data in all_trends.values()]) * 100
                }
                
                axes[1, 1].bar(style_chars.keys(), style_chars.values(), color=['red', 'green', 'yellow', 'purple'])
                axes[1, 1].set_title('Style Characteristics Distribution')
                axes[1, 1].set_ylabel('Percentage (%)')
                axes[1, 1].tick_params(axis='x', rotation=45)
            
            plt.tight_layout()
            plt.savefig(f"{self.output_dir}/reports/trend_summary_{datetime.now().strftime('%Y%m%d')}.png", 
                       dpi=150, bbox_inches='tight')
            plt.close()
            
        except Exception as e:
            print(f"[visualization] Error creating trend visualization: {e}")
    
    def run_full_analysis(self, days_back=30):
        """Run complete visual trend analysis"""
        print("=== Starting Full Visual Trend Analysis ===")
        
        start_time = datetime.now()
        
        try:
            # Main analysis
            trend_report = self.analyze_trending_visuals(days_back=days_back)
            
            # Print summary
            print("\n=== ANALYSIS SUMMARY ===")
            if trend_report['summary']:
                print(f"Best Icon Style: {trend_report['summary'].get('best_icon_style', 'N/A')}")
                print(f"Best Thumbnail Style: {trend_report['summary'].get('best_thumbnail_style', 'N/A')}")
            
            print(f"\nTop Icon Styles: {len(trend_report['top_icon_styles'])}")
            print(f"Top Thumbnail Styles: {len(trend_report['top_thumbnail_styles'])}")
            print(f"Recommendations Generated: {len(trend_report['visual_recommendations'])}")
            
            end_time = datetime.now()
            print(f"\nAnalysis completed in {end_time - start_time}")
...
if __name__ == "__main__":
    ...
    report = analyzer.run_full_analysis(days_back=30)
    
    if report:
        print("\n=== SUCCESS ===")
        print("Visual trend analysis completed successfully!")
        print(f"Reports saved to: {analyzer.output_dir}/reports/")
    else:
        print("\n=== FAILED ===")
        print("Visual trend analysis failed. Check logs for details.")
